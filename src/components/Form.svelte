<script>
  ...
  const formProps = {
    ...
    onSubmit: values => {
      ...
      // const message = `y%20este%20es%20mi%20mensaje:%0D%0A${values.message.split(' ').join('%20')}`;
      // window.open(`https://wa.me/${phone}?text=${name},%20${mail}%20${message}`, '_blank');

      console.log('handling whatsapp G version');
      // check if directo api o wwhatsapp web
